...
export default function ProductCard({ product, size = 'md', showFrom = false }: ProductCardProps) {
  ...
  return (
    ...
    >
      {p.image ? (
        <div className={`relative ${imageHeight} overflow-hidden bg-surface-light`}>
          <SafeImage
            src={p.image}
            alt={p.name}
            fill
          ...
          />
          <div className="product-card-action absolute inset-0 bg-navy-700/50 flex items-center justify-center opacity-0 transition-opacity duration-300">
            <span className="text-white text-sm font-semibold px-4 py-2 bg-navy-500">
              View Deal
            </span>
          </div>
        </div>
      ) : (
        <div className={`${imageHeight} bg-surface-light flex items-center justify-center`}>
          <span className="text-gray-400 text-sm">No image</span>
        </div>
      )}
      <div className="p-3">
        <p className="text-[11px] text-gray-400 uppercase tracking-wide font-display">{p.brand}</p>
        <h4 className="font-medium text-sm text-gray-800 group-hover:text-navy-500 transition-colors leading-tight mt-0.5 line-clamp-2">
          {p.name}
        </h4>
